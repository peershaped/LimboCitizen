...
function asyncFunc(storypart, callback) {
	...
	if(storypartobject.audio.id != '' || storypartobject.stopaudio === true){
		...
		if(storypartobject.audio.untilfinish === true)
		{
			path = '/play/'+storypartobject.audio.id;
		}

		if(storypartobject.audio.untilfinish === false)
		{
			path = '/loop/'+storypartobject.audio.id;
		}

		if(storypartobject.stopaudio === true){
			path = '/stop';
		}
